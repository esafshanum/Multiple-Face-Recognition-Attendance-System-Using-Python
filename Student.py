...
class Student:
    def __init__(self,root):
        ...
        dept_combo["values"]=("Select Department","Computer Engineering","Mechanical Engineering","AI&DS Engineering","EXTC Engineering","Civil Engineering","Information Technology")
        ...
        course_combo["values"]=("Select Course","FE","SE","DSE","TE","BE")
    ...
    def validate_fields(self):
        ...
        if self.var_Year.get() == "Select Year":
            messagebox.showerror("Error", "Please select a Year", parent=self.root)
            return False
        ...
        # Check empty fields
        required_fields = {
            "Enrollment Number": self.var_Enrollment_No.get(),
            "Student Name": self.var_Student_Name.get(),
            "Class": self.var_Class.get(),
            "Roll No": self.var_Roll_No.get(),
            "Date of Birth": self.var_Date_Of_Birth.get(),
            "Contact No": self.var_Contact_No.get(),
            "Address": self.var_Address.get(),
            "Coordinator": self.var_Coordinator.get(),
            "Email ID": self.var_Email_ID.get()
        }
        for field, value in required_fields.items():
            if value.strip() == "":
                messagebox.showerror("Error", f"{field} cannot be empty", parent=self.root)
                return False

     # Validate numeric fields
        if not self.var_Enrollment_No.get().isdigit():
            messagebox.showerror("Error", "Enrollment Number must be numeric", parent=self.root)
            return False
        if not self.var_Roll_No.get().isdigit():
            messagebox.showerror("Error", "Roll Number must be numeric", parent=self.root)
            return False
        if not self.var_Contact_No.get().isdigit() or len(self.var_Contact_No.get()) != 10:
            messagebox.showerror("Error", "Contact Number must be 10 digits", parent=self.root)
            return False

        # Validate student name
        if not re.match(r"^[A-Za-z\s]+$", self.var_Student_Name.get()):
            messagebox.showerror("Error", "Student name must contain only alphabets", parent=self.root)
            return False

        # Validate DOB format
        try:
            datetime.strptime(self.var_Date_Of_Birth.get(), "%d/%m/%Y")
        except ValueError:
            messagebox.showerror("Error", "Date of Birth must be in DD/MM/YYYY format", parent=self.root)
            return False

        # Validate Email
        if not re.match(r"[^@]+@[^@]+\.[^@]+", self.var_Email_ID.get()):
            messagebox.showerror("Error", "Invalid Email ID format", parent=self.root)
            return False
